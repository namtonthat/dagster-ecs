"""
Dynamic workspace loader for Dagster that automatically discovers all DAG files.
This module scans the dags/ directory and loads all assets into a single Definitions object.
"""
...
def load_assets_from_file(file_path: Path) -> list[Any]:
    ...
    try:
        ...
        if assets:
            relative_path = file_path.relative_to(Path("/app/dags"))
...
# Discover all assets when the module is imported
all_assets = []
dags_dir = Path("/app/dags")
